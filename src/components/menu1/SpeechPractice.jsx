      ...
      <div className="flex-1 overflow-y-auto max-w-2xl w-full mx-auto px-4 py-4 space-y-3">
        ...
        {isListening && liveText && !loading ? (
          <div className="flex justify-end">
            ...
            <div className="w-8 h-8 rounded-full bg-blue-400 flex items-center justify-center ml-2 flex-shrink-0 mt-1 opacity-80">
              <User size={15} className="text-white" />
            </div>
          </div>
        ) : isListening && !loading ? (
          /* 마이크 활성 중 — 말하기 전 대기 인디케이터 */
          <div className="flex justify-end">
            <div className="px-4 py-3 rounded-2xl bg-blue-50 border border-blue-100 flex items-center gap-1.5">
              {[0,1,2].map(i => (
                <span key={i} className="w-2 h-2 bg-blue-300 rounded-full animate-bounce" style={{animationDelay:`${i*0.15}s`}} />
              ))}
            </div>
          </div>
        ) : null}
