...
평가 기준: 오프닝 임팩트, 니즈 파악, 가치 제안, 반론 처리, 클로징 시도, 대화 흐름

[필수 규칙]
1. 순수 JSON만 반환 (마크다운 코드블록 금지)
2. JSON 문자열 값 안에 줄바꿈 절대 금지 — 각 항목은 반드시 한 줄
3. 큰따옴표 안에 큰따옴표 사용 금지

반환할 JSON 구조 (이 형식 그대로):
{"score":75,"summary":"전반적 평가 2~3문장","good":["잘한점1","잘한점2","잘한점3","잘한점4","잘한점5"],"improve":["개선점1","개선점2","개선점3","개선점4","개선점5"],"scripts":["상황: 상황설명 → 멘트: 실제멘트","상황: 상황설명 → 멘트: 실제멘트","상황: 상황설명 → 멘트: 실제멘트","상황: 상황설명 → 멘트: 실제멘트","상황: 상황설명 → 멘트: 실제멘트"]}
...
위 구조를 그대로 따르되 실제 분석 내용으로 채워서 반환하세요.`;
  ...
  // JSON 블록 추출 (앞뒤 불필요한 텍스트 제거)
  const jsonMatch = text.match(/\{[\s\S]*\}/);
  if (jsonMatch) text = jsonMatch[0];

  // JSON 문자열 내 개행·탭 제거 (Gemini가 값 안에 \n을 넣으면 parse 실패)
...

{
  ...
  "salesMethod": {
    ...
    "realCases": [
      {
        "company": "【세일즈맨 사례】 판매 상품(${product})과 동일하거나 유사한 업종에서 활동한 실존 세일즈맨. 조 지라드·프랭크 베트거·론 포펄 같은 유명인을 피하고, 업종에 딱 맞는 현장형 인물(보험, 부동산, IT영업, 식품 유통, 의약품 영업 등 업종별 고수)을 찾으세요. 한국 세일즈 고수도 적극 포함하세요.",
        "product": "그 세일즈맨이 실제로 팔았던 상품/업종 (판매 상품(${product})과 연결점 명시)",
        "before": "그가 처음 겪은 현실적 어려움 또는 기존의 평범한 방식",
        "after": "독창적 접근으로 이룬 구체적 성과 (숫자·기록 포함)",
        "lesson": "이 세일즈맨의 핵심 방식을 현재 상품(${product}) 판매에 내일 당장 적용할 수 있는 구체적 행동"
      },
      {
        "company": "【스타트업 사례】 판매 상품(${product})과 동일하거나 인접 시장에서 혁신적 세일즈 방식으로 급성장한 실존 스타트업 회사명. 국내외 모두 가능하며, 반드시 ${product} 업종과 연결되는 회사를 선택하세요.",
        "product": "이 스타트업이 판매한 상품/서비스 (판매 상품(${product})과 유사성 명시)",
        "before": "초기 자원 부족·시장 진입 장벽 등 현실적 어려움",
        "after": "독창적 영업·마케팅 방식으로 이룬 성장 수치 또는 시장 성과",
        "lesson": "이 스타트업의 영업 전략 핵심을 현재 상품(${product}) 세일즈에 내일 당장 적용하는 구체적 행동"
      },
      {
        "company": "【기업 사례】 판매 상품(${product})과 동일하거나 인접 업종의 중견·대기업 또는 글로벌 기업 중, 세일즈 조직·채널·방식을 혁신해 성과를 낸 실존 회사. 국내외 모두 가능, 반드시 ${product} 업종과 연결되는 회사를 선택하세요.",
        "product": "이 기업이 판매한 주력 상품/서비스 (판매 상품(${product})과 연결점 명시)",
        "before": "기존의 평범한 영업 방식 또는 시장 침체 상황",
        "after": "새로운 세일즈 전략으로 이룬 구체적 매출 성과 또는 시장 점유율 변화",
        "lesson": "이 기업의 세일즈 혁신 방식을 개인 세일즈맨 입장에서 현재 상품(${product}) 판매에 내일 당장 적용하는 구체적 행동"
      }
    ]
  }
}`;
